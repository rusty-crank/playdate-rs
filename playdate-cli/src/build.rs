...
impl Build {
    fn build_setup(&self, target_dir: &Path) -> anyhow::Result<PathBuf> {
        ...
        Command::new("arm-none-eabi-gcc")
            ...
            .args("-std=gnu11 -Wall -Wno-unknown-pragmas -Wdouble-promotion -mthumb -mcpu=cortex-m7 -mfloat-abi=hard -mfpu=fpv5-sp-d16 -D__FPU_USED=1 -falign-functions=16 -fomit-frame-pointer -gdwarf-2 -fverbose-asm -ffunction-sections -fdata-sections -mword-relocations -fno-common -MD -MT".split(' ').map(|s| s.to_owned()))
    ...
    fn link_arm_binary(
        ...
        let linker_script = crate::util::get_playdate_sdk_path()?
            ...
            .join("link_map.ld");
        Command::new("arm-none-eabi-gcc")
        .args("-nostartfiles -mthumb -mcpu=cortex-m7 -mfloat-abi=hard -mfpu=fpv5-sp-d16 -D__FPU_USED=1".split(' ').map(|s| s.to_owned()))
        .arg("-T")
        .arg(linker_script )
        .args("-Wl,-Map=game.map,--cref,--gc-sections,--no-warn-mismatch,--emit-relocs".split(' ') )
